...
public class QuadraticProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        for (int i = 0; i < table.length; i++) {
            int curr = (index + (i * i) % table.length);
            QuadraticProbingMapEntry<K, V> entry = table[curr];

            if (entry == null || entry.isRemoved()) {
                table[curr] = new QuadraticProbingMapEntry<K, V>(key, value);
                size++;
                return null;
    ...
    public V remove(K key) {
        if (key == null) {
            throw new IllegalArgumentException("Key given is not valid.");
        } else if (!containsKey(key)) {
            throw new NoSuchElementException("Key was not found in the map.");
        }
        ...
        for (int i = 1; i <= table.length; i++) {
            ...
            if (entry != null) {
                if (!entry.isRemoved() && entry.getKey().equals(key)) {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("Length given is less than the size of the map.");
        }
        ...
        for (QuadraticProbingMapEntry<K, V> entry : table) {
            if (entry != null && !entry.isRemoved()) {
                ...
                insertIntoTable(newTable, entry, index);
            }
        }
    ...
    /**
     * Private helper method to insert values into a new backingTable.
     * 
     * @param table new table to insert values into.
     * @param entry object to insert into table.
     * @param index where to insert the entry at in the table.
     */
    private void insertIntoTable(QuadraticProbingMapEntry<K, V>[] table, QuadraticProbingMapEntry<K, V> entry,
            int index) {
        if (table[index] == null) {
            table[index] = entry;
        } else {
